...
def send_img():
    global count
    count += 1
    pil_img.save("PRINT" + str(count) + ".png")
    draw_img("PRINT" + str(count) + ".png")
...
file_upload.grid(row=1, column=2, padx=5, pady=5)
slider.grid(row = 1, column = 1, pady = 5)
send.grid(row=0, column=0, padx=5, pady=5)
